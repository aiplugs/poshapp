...
if (-not (Test-Path -Path ./build/PSScriptAnalyzer.zip)) {
    Invoke-RestMethod https://github.com/PowerShell/PSScriptAnalyzer/releases/download/1.19.1/PSScriptAnalyzer.zip -OutFile ./build/PSScriptAnalyzer.zip
}
mkdir ./bin/pses/bin/Common/Modules
Expand-Archive -Path ./build/PSScriptAnalyzer.zip -DestinationPath ./bin/pses/bin/Common/Modules/PSScriptAnalyzer
